...
1. Generate the genesis file and the initial consensus protocol group files
   ...
   - The initial consensus protocol group files contain the credentials for all the participating nodes and
     ...
     [White Paper](https://github.com/NOVAwang/dex/wiki/White-Paper) for details.
...
1. Start nodes.
   ...
   1. Start node 0 on port 9000, wallet RPC service is on port 12000
      `$ make node node=0 port=9000 rpc-addr=12000`
   1. Start node 1 on port 9001, wallet RPC service is on port 12001, use `:9000` as the seed node
      `$ make node node=1 port=9001 rpc-addr=12001 seed=9000`
   1. Start node 2 on port 9002, wallet RPC service is on port 12002, use `:9000` as the seed node
      `$ make node node=2 port=9002 rpc-addr=12002 seed=9000`  
...
Sell 15 ETH at 0.07 NOVA, expire after 3000 blocks:

`$ make wallet node=0 cmd="order ETH_NOVA sell 0.07 15 3000"`

Check account:

```
$ make wallet node=0 cmd="account"
Addr:
...
Balances:
 ...
 |NOVA   |9000000.00000000 |0.00000000  |       |

Pending Orders:
 |ID    |Market  |Side |Price      |Amount      |Executed   |Expiry Block Height |
 |2_1_0 |ETH_NOVA |SELL |0.07000000 |15.00000000 |0.00000000 |3005                |

Execution Reports:
 |Block |ID |Market |Side |Trade Price |Amount |
```

Buy 10 ETH at 0.08 NOVA, expire after 3000 blocks:

`$ make wallet node=0 cmd="order ETH_NOVA buy 0.08 10 3000"`
...
Pending Orders:
 |ID    |Market  |Side |Price      |Amount      |Executed    |Expiry Block Height |
 |2_1_0 |ETH_NOVA |SELL |0.07000000 |15.00000000 |10.00000000 |3005                |

Execution Reports:
 |Block |ID    |Market  |Side |Trade Price |Amount      |
 |31    |2_1_1 |ETH_NOVA |BUY  |0.07000000  |10.00000000 |
 |31    |2_1_0 |ETH_NOVA |SELL |0.07000000  |10.00000000 |
```
...
`$ make wallet node=0 cmd="cancel 2_1_0"`

Please note that cancelling an order will not generate an execution report.

### Issue Token

Issue NOVA-COIN, total supply 999999, decimals 8:

`$ make wallet node=0 cmd="issue_token NOVA-COIN 999999 8"`

### List All Tokens

```
$ make wallet cmd="token"
...
1. Get the public key of the account 1
   ```
   $ make credential_info node=1
   credential info (bytes encoded using base64):
   SK: hDTgUQxmwGCaG/abozy/iIMHiT1S3OtlxFAa5TRmmRU=
   PK: BIuJuatFzQMT052XleCjB7ovobfZQOw0Kwn844Z8SlDRSPJ3wm+y/dkA46kvzeuZIe20oJCwnSWiras0pm4rZoc=
   Addr: c09676fdec88c1e960e6398f1c281defdd1cb4fa
   ```
1. Send to account 1's public key:

   ```
   $ make wallet node=0 cmd="send BIuJuatFzQMT052XleCjB7ovobfZQOw0Kwn844Z8SlDRSPJ3wm+y/dkA46kvzeuZIe20oJCwnSWiras0pm4rZoc= NOVA-COIN 20"
   ```

   Verify account 1 received it:

   ```
   $ make wallet node=1 cmd="account"
   Addr:
   ...
   Balances:
    ...
    |BCC        |9000000.00000000 |0.00000000 |       |
    |LTC        |9000000.00000000 |0.00000000 |       |
    |NOVA-COIN  |20.00000000      |0.00000000 |       |
...
```
$ make wallet node=0 cmd="freeze BNB 10000 500"
...
```
$ make wallet node=0 cmd="burn BTC 1000"
```
